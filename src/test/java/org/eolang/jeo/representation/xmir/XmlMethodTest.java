...
final class XmlMethodTest {
            ...
            new XmlMethod(
                new NativeXmlNode(
                    new Xembler(
                        new DirectivesMethod(
                            name,
                            new DirectivesMethodProperties(access, name, descriptor, "")
                        )
                    ).xmlQuietly()
                )
            ).bytecode(),
            ...
            new XmlMethod(
                new NativeXmlNode(
                    new Xembler(
                        new DirectivesMethod(
                            name,
                            new DirectivesMethodProperties(access, name, descriptor, "", exceptions)
                        )
                    ).xmlQuietly()
                )
            ).bytecode(),
                    ...
                    new Xembler(
                        new DirectivesMethod(
                            ...
                            new DirectivesMethodProperties(
                                ...
                                "",
                                new String[0],
                                new DirectivesMaxs(new Format(), 1, 2),
    ...
    void parsesWithExceptions() throws ImpossibleModificationException {
        final BytecodeMethod original = new BytecodeMethod("exceptionsAndFailedTasks").trycatch(
